...
def generate_sequence_data(num_samples=1000, seq_len=10, input_dim=10, output_dim=5):
    ...
    # Create a multi-step task - simpler implementation
    # Use a linear combination of all timesteps
    weights = torch.randn(seq_len * input_dim, output_dim)
    bias = torch.randn(output_dim)
    
    # Flatten the sequence dimension
    X_flat = X.reshape(num_samples, -1)
    y = torch.tanh(torch.matmul(X_flat, weights) + bias)
...
def main():
    ...
    # Test configurations - start with fewer to test
    configurations = []
