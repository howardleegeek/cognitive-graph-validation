#!/usr/bin/env python3
"""
Fixed version of H1.431 experiment.
"""
...
class SimpleCG(nn.Module):
    def __init__(self, robot_dim, object_dim, n_objects, output_dim, hidden_dim=128):
        ...
        # Decoder
        decoder_input_dim = hidden_dim * (1 + n_objects + n_objects * (n_objects - 1) // 2)
    ...
    def forward(self, x):
        ...
        object_embs = self.object_net(object_features.reshape(-1, object_dim))  # [batch*n_objects, hidden]
        object_embs = object_embs.reshape(batch_size, self.n_objects, -1)  # [batch, n_objects, hidden]
...
def main():
    print("H1.431 Experiment: Testing CG on Relational Tasks")
    print("=" * 60)
