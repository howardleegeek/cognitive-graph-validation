...
class BaselineArchitecture(nn.Module):
    ...
    def forward(self, obs, lang):
        # obs shape: (batch, seq_len, obs_dim) or (batch, obs_dim)
        # lang shape: (batch, lang_dim)
        
        # If obs has seq_len dimension, process each timestep
        if obs.dim() == 3:
            batch_size, seq_len, obs_dim = obs.shape
            # Process each timestep independently
            obs_flat = obs.reshape(-1, obs_dim)
            obs_enc_flat = self.obs_encoder(obs_flat)
            obs_enc = obs_enc_flat.reshape(batch_size, seq_len, -1)
            
            # Expand language for each timestep
            lang_expanded = lang.unsqueeze(1).expand(-1, seq_len, -1)
            lang_flat = lang_expanded.reshape(-1, lang_dim)
            lang_enc_flat = self.lang_encoder(lang_flat)
            lang_enc = lang_enc_flat.reshape(batch_size, seq_len, -1)
            
            # Concatenate and decode
            combined = torch.cat([obs_enc, lang_enc], dim=-1)
            combined_flat = combined.reshape(-1, combined.shape[-1])
            output_flat = self.fusion(combined_flat)
...
class CognitiveGraphArchitecture(nn.Module):
    ...
    def forward(self, obs, lang):
        ...
        if obs.dim() == 3:
            ...
            z_phys_flat = self.obs_to_unified(obs_flat)
            z_phys = z_phys_flat.reshape(batch_size, seq_len, self.physical_dim)
            
            # Expand language for each timestep
            lang_expanded = lang.unsqueeze(1).expand(-1, seq_len, -1)
            lang_flat = lang_expanded.reshape(-1, lang.shape[-1])
            z_sem_flat = self.lang_to_unified(lang_flat)
            z_sem = z_sem_flat.reshape(batch_size, seq_len, self.semantic_dim)
            ...
            # Combine physical and semantic nodes
            # Shape: (batch, seq_len, 2, total_dim)
            nodes = torch.stack([z_phys_pad, z_sem_pad], dim=2)
            
            # Process through GNN layers for each timestep
            for layer in self.gnn_layers:
                # Mean pooling across node dimension
                msgs = nodes.mean(dim=2, keepdim=True).expand(-1, -1, 2, -1)
                nodes = nodes + layer(msgs)
            
            # Reshape for attention: (batch * seq_len, 2, total_dim)
            nodes_flat = nodes.reshape(batch_size * seq_len, 2, -1)
            
            # Cross-modal attention
            attn_out_flat, _ = self.cross_attn(nodes_flat, nodes_flat, nodes_flat)
            
            # Decode
            output_flat = self.decoder(attn_out_flat.mean(dim=1))
            output = output_flat.reshape(batch_size, seq_len, -1)
