...
for seq_len in [420, 430, 440]:
    ...
    results[seq_len] = {"attn": attn_loss, "concat": concat_loss, "delta": delta}
    if delta > 0:
        attn_wins += 1
    print(f"Seq={seq_len}: Attn={attn_loss:.4f}, Concat={concat_loss:.4f}, Δ={delta:+.1f}%")

avg_delta = np.mean([r["delta"] for r in results.values()])
print(f"\n=== SUMMARY ===")
print(f"Avg improvement: {avg_delta:+.1f}%")
print(f"Attention wins: {attn_wins}/3")

status = "SUPPORTED" if avg_delta > 0 and attn_wins >= 2 else "REFUTED"
print(f"Status: {status}")

with open("results.json", "w") as f:
    json.dump({"results": results, "avg_delta": avg_delta, "attn_wins": attn_wins, "status": status}, f, indent=2)