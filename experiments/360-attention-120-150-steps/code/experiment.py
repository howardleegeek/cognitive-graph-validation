#!/usr/bin/env python3
"""
H3.360: Attention on 120-150 Step Sequences (Find Upper Boundary)
Based on H3.359: Attention +21.8% on 80-100 steps
Test if attention advantage continues or collapses at 120-150 steps
"""
...
class ExtremeLongSequenceDataset(Dataset):
    """Dataset with extreme long sequences (120-150 timesteps)"""
    def __init__(self, base_dataset, seq_len=135):
    ...
    def __getitem__(self, idx):
        ...
        for t in range(self.seq_len):
            phase = (t % 25) / 25.0
            obs_t = base_obs + np.sin(phase * np.pi) * 0.3 + np.random.randn(*base_obs.shape).astype(np.float32) * 0.02
            action_t = base_action + np.sin(t * 0.08) * 0.18
...
results = {}
for seq_len in [120, 135, 150]:
    print(f"\n=== Testing {seq_len}-step sequences ===")
    train_seq = ExtremeLongSequenceDataset(train_data, seq_len=seq_len)
    val_seq = ExtremeLongSequenceDataset(val_data, seq_len=seq_len)
...
output = {
    ...
    'config': {
        'task_type': 'attention_extreme_long_sequences',
