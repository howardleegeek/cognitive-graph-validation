#!/usr/bin/env python3
"""
H3.145: Causal Attention on 60-80 Step Sequences

Based on findings:
- H1.247: Hierarchical attention +7.7% on 50-80 steps
- H1.248: Hierarchical attention +5.8% on 80-100 steps
- H3.144: Chunked attention -7.4% on 50+ steps

Hypothesis: Causal attention (only attending to past) may work better than 
full attention on longer sequences by enforcing temporal ordering.
"""
...
class CausalAttentionModel(nn.Module):
    ...
    def forward(self, obs, lang):
        ...
        mask = torch.triu(torch.ones(seq_len, seq_len, device=combined.device), diagonal=1).bool()
        
        attn_out, _ = self.attn(combined, combined, combined, attn_mask=mask)
...
class StandardAttentionModel(nn.Module):
    """Standard (non-causal) attention for comparison"""
    def __init__(self, obs_dim=8, lang_dim=32, action_dim=7, hidden_dim=256, num_heads=8, reg=0.35):
        super().__init__()
        self.reg = reg
...
def run_experiment():
    """Run the causal attention experiment"""
    print("=" * 70)
    print("H3.145: Causal Attention on 60-80 Step Sequences")
    print("=" * 70)
    
    results = {}
    
    seq_lengths = [60, 70, 80]
    autocorr = 0.95
    ...
    for seq_len in seq_lengths:
        ...
        causal_model = CausalAttentionModel(reg=0.35)
        causal_loss = train_model(causal_model, train_loader, val_loader)
        ...
        causal_improvements.append(causal_improvement)
        std_improvements.append(std_improvement)
        
        print(f"  Baseline: {base_loss:.6f}")
        print(f"  Causal: {causal_loss:.6f} ({causal_improvement:+.1f}%)")
    ...
    avg_causal = np.mean(causal_improvements)
    avg_std = np.mean(std_improvements)
    
    print("\n" + "=" * 70)
    print("RESULTS SUMMARY")
    print("=" * 70)
    print(f"Average Causal Improvement: {avg_causal:+.1f}%")
    print(f"Average Standard Attention Improvement: {avg_std:+.1f}%")
    print(f"Causal vs Standard: {avg_causal - avg_std:+.1f}%")
    
    if avg_causal > avg_std and avg_causal > 0:
        status = "SUPPORTED"
        print(f"\nStatus: {status} - Causal attention outperforms standard on 60-80 steps")
    elif avg_std > avg_causal and avg_std > 0:
        status = "REFUTED"
    ...
    results['summary'] = {
        'avg_causal_improvement': avg_causal,
        'avg_std_improvement': avg_std,
        'status': status,
        'causal_wins': sum(1 for c in causal_improvements if c > 0),
        'std_wins': sum(1 for s in std_improvements if s > 0),
        'causal_better_than_std': sum(1 for i in range(len(seq_lengths)) if causal_improvements[i] > std_improvements[i])
    }
    
    with open('/Users/howardli/Downloads/oyster/products/oyster-world/research/cognitive-graph-validation/experiments/H3.145-causal-attention-60-80-steps/results/metrics.json', 'w') as f:
