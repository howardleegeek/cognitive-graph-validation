...
Fixed version: Use fixed observation dimension for all demonstrations.
"""
...
class HierarchicalPlannerArchitecture(nn.Module):
    ...
    def __init__(self, obs_dim=64, lang_dim=32, action_dim=7, latent_dim=128, n_subgoals=2):
        super().__init__()
...
class CognitiveGraphSmallArchitecture(nn.Module):
    ...
    def __init__(self, obs_dim=64, lang_dim=32, action_dim=7, n_heads=1, n_layers=1):
        super().__init__()
...
class CognitiveGraphLargeArchitecture(nn.Module):
    ...
    def __init__(self, obs_dim=64, lang_dim=32, action_dim=7, n_heads=4, n_layers=2):
        super().__init__()
...
def generate_real_robot_data(n_demos=1200, max_objects=8, task_complexity='simple'):
    ...
    # Fixed observation dimension (8 per object * max_objects)
    obs_dim = 8 * max_objects
    ...
    for demo_idx in range(n_demos):
        ...
        # Initial state (only first n_objects are active, rest are zero)
        current_state = np.zeros(obs_dim)
        for obj_idx in range(n_objects):
            start_idx = obj_idx * 8
            current_state[start_idx:start_idx+8] = np.random.randn(8) * 0.1
        ...
        for t in range(seq_length):
            ...
            # Simple dynamics: update active objects
            for obj_idx in range(n_objects):
                start_idx = obj_idx * 8
                # Only affect first object (robot arm) with full action
                if obj_idx == 0:
                    current_state[start_idx:start_idx+7] += action[:7]
                    current_state[start_idx+7] = action[6]  # Gripper state
                else:
                    # Other objects get smaller updates
                    current_state[start_idx:start_idx+7] += np.random.randn(7) * 0.1
            
            # Add noise
            current_state += np.random.randn(obs_dim) * noise_level
        ...
        lang_embedding = np.random.randn(32)
    ...
    return demonstrations, obs_dim

def prepare_data_loaders(demonstrations, obs_dim, batch_size=32):
...
def main():
    ...
    for complexity in config['complexity_levels']:
        ...
        demonstrations, obs_dim = generate_real_robot_data(
        ...
        train_loader, val_loader, test_loader = prepare_data_loaders(
            demonstrations, obs_dim, batch_size=config['batch_size']
        )
