...
class CognitiveGraphWithSupervision(nn.Module):
    ...
    def __init__(self, obs_dim=8, lang_dim=32, action_dim=7, physical_dim=144, semantic_dim=368, n_subgoals=2):
        ...
        self.subgoal_predictor = nn.Sequential(
            ...
            nn.Linear(256, 128),
            nn.ReLU(),
            nn.Linear(128, n_subgoals * obs_dim)  # Predict subgoals in observation space
        )
    ...
    def forward(self, obs, lang):
        ...
        subgoal_pred = subgoal_pred.view(-1, self.n_subgoals, 8)  # obs_dim = 8
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            if model_type == 'hierarchical':
                ...
                # Generate synthetic subgoal targets (random in observation space)
                # This simulates having intermediate state supervision
                batch_size = batch['observation'].shape[0]
                subgoal_targets = torch.randn(batch_size, 2, 8) * 0.1 + batch['observation'].unsqueeze(1)
    ...
    for name, model in models.items():
        ...
        if name == 'cg':
            ...
            results[name] = {
                ...
                'decomposition_variance': np.var(decompositions, axis=0).mean()
            }
...
def main():
    ...
    if 'cg' in representation_results:
        ...
        metrics = ['z_phys_norm', 'z_sem_norm', 'cosine_similarity', 'decomposition_variance']
        values = [cg_data['z_phys_norm'], cg_data['z_sem_norm'], cg_data['cosine_similarity'], cg_data['decomposition_variance']]
    ...
    if 'cg' in representation_results:
        ...
        print(f"  Decomposition variance: {representation_results['cg']['decomposition_variance']:.3f}")
