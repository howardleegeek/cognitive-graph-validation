...
Fixed version with dynamic subgoal predictor.
"""
...
class HierarchicalPlanner(nn.Module):
    """Hierarchical planner with dynamic subgoal decomposition."""
    ...
    def set_n_subgoals(self, n_subgoals):
        """Dynamically change number of subgoals (requires reinitializing predictor)."""
        self.n_subgoals = n_subgoals
        # Reinitialize the last layer of subgoal_predictor
        old_predictor = self.subgoal_predictor
        hidden_dim = old_predictor[2].in_features  # Get hidden dimension
        
        self.subgoal_predictor = nn.Sequential(
            nn.Linear(32 + 8, hidden_dim),  # lang_dim + obs_dim
            nn.ReLU(),
            nn.Linear(hidden_dim, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, n_subgoals * 8)  # n_subgoals * subgoal_dim
        )
        
        # Copy weights from old layers where possible
        with torch.no_grad():
            # Copy first two layers
            self.subgoal_predictor[0].weight.copy_(old_predictor[0].weight)
            self.subgoal_predictor[0].bias.copy_(old_predictor[0].bias)
            self.subgoal_predictor[2].weight.copy_(old_predictor[2].weight)
            self.subgoal_predictor[2].bias.copy_(old_predictor[2].bias)
            
            # Initialize new last layer
            nn.init.xavier_uniform_(self.subgoal_predictor[4].weight)
            nn.init.zeros_(self.subgoal_predictor[4].bias)
...
def run_experiment():
    ...
    direct_dataset = CurriculumMultiStepDataset(
        n_samples=800, n_steps=4, n_subgoals=2, curriculum_mode=False, seed=42
    )
    ...
    curriculum_dataset = CurriculumMultiStepDataset(
        n_samples=800, n_steps=4, n_subgoals=2, curriculum_mode=True, seed=42
    )
    ...
    print("  Architecture adaptation: Switching to 2 subgoals...")
    hier_curriculum_model.set_n_subgoals(2)
    
    # Reinitialize optimizer for new parameters
    hier_curriculum_optimizer = torch.optim.Adam(hier_curriculum_model.parameters(), lr=1e-4)
