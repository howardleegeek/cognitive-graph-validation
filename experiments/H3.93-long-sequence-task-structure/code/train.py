#!/usr/bin/env python3
"""
H3.93: Attention on 50+ Step Sequences WITH Task Structure
Building on H3.91 (+86.6%) and H1.204 (+94.6%)
"""
...
class TaskStructuredDataset:
    """Dataset with full task structure: goal, subgoals, action outcomes, constraints"""
    ...
    def generate(self):
        ...
        for _ in range(self.n_samples):
            ...
            # Subgoals at key points
            n_subgoals = 5
            subgoal_indices = [i * self.seq_len // n_subgoals for i in range(n_subgoals)]
            subgoals = np.array([states[i].copy() for i in subgoal_indices])
            
            # Action outcomes
            actions = np.random.randn(self.seq_len, self.action_dim) * 0.3
            
            # Constraints (safety limits)
            constraints = np.random.randn(self.seq_len, 4) * 0.1
            
            # Full task structure: states + goal + subgoals + actions + constraints
            task_structured = np.concatenate([
                states[:-1],           # current states [seq, 8]
                np.tile(goal, (self.seq_len, 1)),  # goal [seq, 8]
                np.tile(subgoals.flatten(), (self.seq_len, 1)),  # subgoals [seq, 40]
                actions,               # actions [seq, 7]
                constraints,          # constraints [seq, 4]
            ], axis=1)
...
def main():
    print("=" * 70)
    print("H3.93: Attention on 50+ Step Sequences WITH FULL Task Structure")
    print("=" * 70)
    ...
    final = {
        'experiment': 'H3.93',
        'description': 'Attention on 50+ steps WITH FULL task structure',
    ...
    with open('experiments/H3.93-long-sequence-task-structure/results/metrics.json', 'w') as f:
