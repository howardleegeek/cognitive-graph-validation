#!/usr/bin/env python3
"""
H3.142: Attention on Longer Sequences 27-35 Steps
Based on:
- H1.242: +73.5% on 26-30 steps, drops at 30 (41.6%)
- H3.141: -0.1% on 25-35 steps (REFUTED)
- H1.243: +75.1% on 18-26 steps (SUPPORTED)

Hypothesis: Find exact boundary where attention fails - between 26-30
Testing different regularization to try to extend the range.
"""
...
class LongerSequenceDataset(Dataset):
    """Dataset with longer 27-35 step tasks."""
    
    def __init__(self, n_samples=500, n_steps_list=[27, 28, 30, 32, 35]):
        ...
        for _ in range(n_samples):
            ...
            rho = np.random.uniform(0.85, 0.92)  # Slightly lower autocorrelation
...
class UnifiedAttentionRegArchitecture(nn.Module):
    ...
    def __init__(self, obs_dim=8, lang_dim=32, action_dim=7, 
                 physical_dim=112, semantic_dim=400, reg=0.25):
        super().__init__()
        self.reg = reg
...
def pad_sequence(seq, max_len=35):
    if len(seq) < max_len:
...
def main():
    print("=" * 70)
    print("H3.142: Attention on Longer Sequences 27-35 Steps")
    print("Testing where exactly attention fails - beyond the sweet spot")
    print("=" * 70)
    
    train_data = LongerSequenceDataset(n_samples=500, n_steps_list=[27, 28, 30, 32, 35])
    val_data = LongerSequenceDataset(n_samples=100, n_steps_list=[27, 28, 30, 32, 35])
    
    print(f"\nTrain samples: {len(train_data)}")
    print(f"Val samples: {len(val_data)}")
    print(f"Step lengths: [27, 28, 30, 32, 35]")
    ...
    # 2. Unified + Attention + Reg=0.25 (optimal from H1.243)
    print("\n[2/3] Training Unified+Attention+Reg=0.25...")
    ...
    # 3. Unified + Attention + Reg=0.30 (higher regularization)
    print("\n[3/3] Training Unified+Attention+Reg=0.30...")
    ...
    print("\n--- Per-Step-Length Analysis ---")
    step_improvements = {}
    for n_steps in [27, 28, 30, 32, 35]:
        train_ds = LongerSequenceDataset(n_samples=200, n_steps_list=[n_steps])
        val_ds = LongerSequenceDataset(n_samples=50, n_steps_list=[n_steps])
        
        base = BaselineArchitecture()
        base_loss = train_and_evaluate(base, train_ds, val_ds, epochs=50, is_unified=False)
        
        unified = UnifiedAttentionRegArchitecture(reg=0.25)
    ...
    # Save results
    output = {
        'experiment_id': 'H3.142',
        'hypothesis': 'H3.142',
        'description': 'Attention on longer sequences 27-35 steps',
        'result': {
        ...
        'status': status,
        'note': f'Attention on 27-35 steps: +{avg_improvement:.1f}% avg, boundary test',
