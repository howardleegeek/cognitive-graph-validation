...
def generate_real_robot_like_data(n_samples=1000, seq_length=40, noise_level=0.1):
    ...
    # Base signal (smooth trajectories)
    ...
    for i in range(1, seq_length):
        noise[i] = 0.7 * noise[i-1] + np.random.randn(n_samples) * noise_level
    
    # Add occasional outliers (5% of samples)
    outlier_mask = np.random.rand(seq_length, n_samples) < 0.05
    ...
    noise = noise.T * heteroscedastic_factor
    
    # Combine signal and noise
    data = base_signal.T + noise
    ...
    # Create features (simplified CNN features)
    features = np.random.randn(n_samples, 32) * 0.5 + data.mean(axis=1, keepdims=True)
    
    # Create targets (next state prediction)
    targets = np.roll(data, -1, axis=1)[:, :-1]  # Predict next timestep
    targets = targets.mean(axis=1, keepdims=True)  # Simplified to single value
