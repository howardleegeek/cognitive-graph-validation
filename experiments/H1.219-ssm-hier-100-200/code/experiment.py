#!/usr/bin/env python3
"""
H1.219: SSM + HierGoals on 100-200 step sequences (DEEPEN H1)

Based on H1.113-114 success (+50-57%), test SSM+HierGoals on mid-range.
Also tests H1.217 finding that SSM beats attention on 200-300 steps.

Hypothesis: SSM+HierGoals will dominate on 100-200 step sequences
"""

import sys
sys.path.insert(0, '/Users/howardli/Downloads/oyster/products/oyster-world/research/cognitive-graph-validation/src')

import torch
import torch.nn as nn
import torch.nn.functional as F
import numpy as np
import json
from torch.utils.data import DataLoader, Dataset


class SyntheticLIBERODataset(Dataset):
    """High-fidelity synthetic robot data with temporal autocorrelation."""
    
    def __init__(self, n_samples=300, seq_len=150, autocorrelation=0.85):
        self.n_samples = n_samples
        self.seq_len = seq_len
        self.rho = autocorrelation
        np.random.seed(42)
        
        self.data = []
        for _ in range(n_samples):
            # Generate smooth trajectory with autocorrelation
            base = np.random.randn(seq_len, 8) * 0.5
            smooth = np.zeros_like(base)
            smooth[0] = base[0]
            for t in range(1, seq_len):
                smooth[t] = self.rho * smooth[t-1] + np.sqrt(1-self.rho**2) * base[t]
            
            obs = smooth + np.random.randn(seq_len, 8) * 0.05
            # Action is derivative + noise, same shape as obs
            act = np.gradient(smooth, axis=0)[:, :7] * 0.5 + np.random.randn(seq_len, 7) * 0.02
            lang = np.random.randn(seq_len, 32)
            
            self.data.append({
                'observation': obs.astype(np.float32),
                'action': act.astype(np.float32),
                'language': lang.astype(np.float32)
            })
    
    def __len__(self):
        return len(self.data)
    
    def __getitem__(self, idx):
        return self.data[idx]


class SSMModel(nn.Module):
    """Simplified SSM - state space model for sequence modeling."""
    
    def __init__(self, obs_dim=8, lang_dim=32, action_dim=7, hidden=128, state_dim=64):
        super().__init__()
        self.obs_proj = nn.Linear(obs_dim, hidden)
        self.lang_proj = nn.Linear(lang_dim, hidden)
        self.hidden_dim = hidden
        self.state_dim = state_dim
        
        # SSM-like recurrence
        self.ssm_A = nn.Parameter(torch.randn(state_dim, state_dim) * 0.01)
        self.ssm_B = nn.Linear(hidden, state_dim, bias=False)
        self.ssm_C = nn.Linear(state_dim, hidden, bias=False)
        
        self.fusion = nn.Sequential(
            nn.Linear(hidden * 2, hidden),
            nn.LayerNorm(hidden),
            nn.GELU(),
        )
        self.out = nn.Linear(hidden, action_dim)
        
    def forward(self, obs, lang, hidden=None):
        # obs: (B, T, D), lang: (B, T, L)
        B, T, _ = obs.shape
        if hidden is None:
            hidden = torch.zeros(B, self.state_dim, device=obs.device)
        
        outputs = []
        for t in range(T):
            x = self.obs_proj(obs[:, t]) + self.lang_proj(lang[:, t])  # (B, hidden)
            hidden = torch.sigmoid(self.ssm_A @ hidden.T + self.ssm_B(x).T).T  # (B, state)
            h = self.ssm_C(hidden)  # (B, hidden)
            out_t = self.out(self.fusion(torch.cat([x, h], dim=-1)))
            outputs.append(out_t)
        
        return torch.stack(outputs, dim=1)


class HierGoalsSSM(nn.Module):
    """SSM with hierarchical goal decomposition."""
    
    def __init__(self, obs_dim=8, lang_dim=32, action_dim=7, hidden=128, state_dim=64, n_levels=3):
        super().__init__()
        self.n_levels = n_levels
        self.hidden_dim = hidden
        self.state_dim = state_dim
        
        self.obs_proj = nn.Linear(obs_dim, hidden)
        self.lang_proj = nn.Linear(lang_dim, hidden)
        
        # Goal extraction at multiple levels
        self.goal_extractors = nn.ModuleList([
            nn.Sequential(nn.Linear(hidden, 32), nn.Tanh()) for _ in range(n_levels)
        ])
        
        # SSM state transition
        self.ssm_A = nn.Parameter(torch.randn(state_dim, state_dim) * 0.01)
        self.ssm_B = nn.Linear(hidden, state_dim, bias=False)
        self.ssm_C = nn.Linear(state_dim, hidden, bias=False)
        
        # Hierarchical fusion: [x_t (hidden), h (hidden), goal (32)]
        self.hier_fusion = nn.Sequential(
            nn.Linear(hidden * 2 + 32, hidden),
            nn.LayerNorm(hidden),
            nn.GELU()
        )
        
        self.out = nn.Linear(hidden, action_dim)
        
    def forward(self, obs, lang):
        B, T, _ = obs.shape
        
        # Project observations and language
        obs_h = self.obs_proj(obs)
        lang_h = self.lang_proj(lang)
        x = obs_h + lang_h
        
        # Extract hierarchical goals at different temporal scales
        goals = []
        for i in range(self.n_levels):
            # Chunk size decreases by factor of 2 each level
            chunk_size = max(1, T // (2 ** (i + 1)))
            n_chunks = T // chunk_size
            if n_chunks > 0:
                # Reshape to (B, n_chunks, chunk_size, hidden) and pool
                chunked = x[:, :n_chunks * chunk_size].reshape(B, n_chunks, chunk_size, -1)
                pooled = chunked.mean(dim=2)  # (B, n_chunks, hidden)
                goal = self.goal_extractors[i](pooled.mean(dim=1))  # (B, 32)
                goals.append(goal)
        
        # SSM processing
        hidden = torch.zeros(B, self.state_dim, device=obs.device)
        outputs = []
        
        # Global goal signal (average of all levels)
        if goals:
            goal_signal = torch.stack(goals, dim=0).mean(dim=0)  # (B, 32)
        else:
            goal_signal = torch.zeros(B, 32, device=obs.device)
        
        for t in range(T):
            x_t = x[:, t]  # (B, hidden)
            
            # SSM update: hidden = sigmoid(A @ hidden + B @ x)
            hidden = torch.sigmoid(self.ssm_A @ hidden.T + self.ssm_B(x_t).T).T
            h = self.ssm_C(hidden)  # (B, hidden)
            
            # Fuse: [x_t (128), h (128), goal (32)] = 288
            fused = torch.cat([x_t, h, goal_signal], dim=-1)  # (B, 288)
            out_t = self.out(self.hier_fusion(fused))
            outputs.append(out_t)
        
        return torch.stack(outputs, dim=1)


class ConcatBaseline(nn.Module):
    """Simple concatenation baseline."""
    
    def __init__(self, obs_dim=8, lang_dim=32, action_dim=7, hidden=128):
        super().__init__()
        self.obs_proj = nn.Linear(obs_dim, hidden)
        self.lang_proj = nn.Linear(lang_dim, hidden)
        self.net = nn.Sequential(
            nn.Linear(hidden * 2, hidden * 2),
            nn.LayerNorm(hidden * 2),
            nn.GELU(),
            nn.Linear(hidden * 2, hidden),
            nn.GELU(),
            nn.Linear(hidden, action_dim)
        )
        
    def forward(self, obs, lang):
        x = torch.cat([self.obs_proj(obs), self.lang_proj(lang)], dim=-1)
        return self.net(x)


def train_model(model, train_loader, epochs=30):
    """Train model and return final loss."""
    opt = torch.optim.AdamW(model.parameters(), lr=1e-3, weight_decay=0.01)
    crit = nn.MSELoss()
    model.train()
    
    for epoch in range(epochs):
        for batch in train_loader:
            opt.zero_grad()
            pred = model(batch['observation'], batch['language'])
            loss = crit(pred, batch['action'])
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            opt.step()
    
    # Evaluate
    model.eval()
    total_loss = 0
    n_batches = 0
    with torch.no_grad():
        for batch in train_loader:
            pred = model(batch['observation'], batch['language'])
            total_loss += crit(pred, batch['action']).item()
            n_batches += 1
    
    return total_loss / n_batches


def main():
    print("=" * 70)
    print("H1.219: SSM + HierGoals on 100-200 step sequences")
    print("=" * 70)
    
    results = {
        'experiment': 'H1.219',
        'description': 'SSM + HierGoals on 100-200 step sequences',
        'results': []
    }
    
    # Test different sequence lengths
    seq_lengths = [100, 125, 150, 175, 200]
    
    for seq_len in seq_lengths:
        print(f"\n--- Testing seq_len={seq_len} ---")
        
        # Create dataset
        train_data = SyntheticLIBERODataset(n_samples=200, seq_len=seq_len, autocorrelation=0.85)
        train_loader = DataLoader(train_data, batch_size=16, shuffle=True)
        
        # Train models
        concat_loss = train_model(ConcatBaseline(), train_loader, epochs=30)
        ssm_loss = train_model(SSMModel(), train_loader, epochs=30)
        hier_loss = train_model(HierGoalsSSM(), train_loader, epochs=30)
        
        # Calculate deltas
        concat_delta = 0  # baseline
        ssm_delta = (concat_loss - ssm_loss) / concat_loss * 100
        hier_delta = (concat_loss - hier_loss) / concat_loss * 100
        
        print(f"  Concat: {concat_loss:.6f} (baseline)")
        print(f"  SSM:    {ssm_loss:.6f} ({ssm_delta:+.1f}%)")
        print(f"  HierSSM: {hier_loss:.6f} ({hier_delta:+.1f}%)")
        
        results['results'].append({
            'seq_len': seq_len,
            'concat_loss': float(concat_loss),
            'ssm_loss': float(ssm_loss),
            'hier_loss': float(hier_loss),
            'ssm_delta': float(ssm_delta),
            'hier_delta': float(hier_delta),
            'ssm_wins': ssm_loss < concat_loss,
            'hier_wins': hier_loss < concat_loss
        })
    
    # Summary
    avg_ssm_delta = np.mean([r['ssm_delta'] for r in results['results']])
    avg_hier_delta = np.mean([r['hier_delta'] for r in results['results']])
    ssm_wins = sum(1 for r in results['results'] if r['ssm_wins'])
    hier_wins = sum(1 for r in results['results'] if r['hier_wins'])
    
    print("\n" + "=" * 70)
    print("SUMMARY")
    print("=" * 70)
    print(f"SSM wins: {ssm_wins}/{len(seq_lengths)} lengths (avg {avg_ssm_delta:+.1f}%)")
    print(f"HierSSM wins: {hier_wins}/{len(seq_lengths)} lengths (avg {avg_hier_delta:+.1f}%)")
    
    # Determine status
    if avg_hier_delta > 0 and hier_wins >= len(seq_lengths) * 0.6:
        status = "SUPPORTED"
        note = "SSM+HierGoals dominates on 100-200 step sequences"
    elif avg_ssm_delta > 0 and ssm_wins >= len(seq_lengths) * 0.6:
        status = "PARTIAL"
        note = "SSM wins, hierarchical adds marginal benefit"
    else:
        status = "REFUTED"
        note = "Concatenation competitive on 100-200 step sequences"
    
    results['summary'] = {
        'status': status,
        'note': note,
        'avg_ssm_delta': float(avg_ssm_delta),
        'avg_hier_delta': float(avg_hier_delta),
        'ssm_wins': ssm_wins,
        'hier_wins': hier_wins
    }
    
    print(f"\nStatus: {status}")
    print(f"Note: {note}")
    
    # Save results
    with open('/tmp/H1.219_results.json', 'w') as f:
        json.dump(results, f, indent=2)
    
    print(f"\nResults saved to /tmp/H1.219_results.json")
    print(json.dumps(results, indent=2))
    
    return results


if __name__ == "__main__":
    main()