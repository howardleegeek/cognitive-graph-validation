#!/usr/bin/env python3
"""
H3.95: Endpoint Goal on Very Long Sequences (100+ steps)
Test if endpoint goal representation enables attention on 100+ step sequences
"""
...
class EndpointGoalDataset:
    """Dataset with endpoint goal representation on long sequences"""
...
def main():
    print("=" * 70)
    print("H3.95: Endpoint Goal on Very Long Sequences (100+ steps)")
    print("=" * 70)
    
    results = []
    seq_lengths = [60, 80, 100, 120, 140]
    n_samples = 200
    
    for seq_len in seq_lengths:
        print(f"\n--- Testing seq_len={seq_len} ---", flush=True)
        
        dataset = EndpointGoalDataset(n_samples=n_samples, seq_len=seq_len)
    ...
    final = {
        'experiment': 'H3.95',
        'description': 'Endpoint goal on very long sequences (100+ steps)',
        ...
        'per_length': {r['seq_len']: r for r in results},
    }
