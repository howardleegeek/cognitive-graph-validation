#!/usr/bin/env python3
"""
H1.220: SSM + HierGoals on 50-100 step sequences (fills gap)

Based on H1.113 (+57%), H1.114 (+50%) on 250-700 steps,
and H1.219 (-8.8%) on 100-200 steps,
test the 50-100 step range to find the crossover point.

Hypothesis: SSM+HierGoals dominates on 50-100 step sequences
"""
...
class SyntheticLIBERODataset(Dataset):
    ...
    def __init__(self, n_samples=300, seq_len=75, autocorrelation=0.85):
...
class SSMModel(nn.Module):
    ...
    def forward(self, obs, lang, hidden=None):
        ...
        outputs = []
        for t in range(T):
            x = self.obs_proj(obs[:, t]) + self.lang_proj(lang[:, t])
            hidden = torch.sigmoid(self.ssm_A @ hidden.T + self.ssm_B(x).T).T
            h = self.ssm_C(hidden)
...
class HierGoalsSSM(nn.Module):
    ...
    def __init__(self, obs_dim=8, lang_dim=32, action_dim=7, hidden=128, state_dim=64, n_levels=3):
        ...
        # Hierarchical fusion: [x_t (128), h (128), goal (32)] = 288
    ...
    def forward(self, obs, lang):
        ...
        for i in range(self.n_levels):
            ...
            if n_chunks > 0:
                ...
                goal = self.goal_extractors[i](pooled.mean(dim=1))
                goals.append(goal)
        ...
        if goals:
            goal_signal = torch.stack(goals, dim=0).mean(dim=0)
        else:
        ...
        for t in range(T):
            ...
            fused = torch.cat([x_t, h, goal_signal], dim=-1)
...
def main():
    print("=" * 70)
    print("H1.220: SSM + HierGoals on 50-100 step sequences")
    print("=" * 70)
    
    results = {
        'experiment': 'H1.220',
        'description': 'SSM + HierGoals on 50-100 step sequences',
        'results': []
    }
    ...
    if avg_hier_delta > 0 and hier_wins >= len(seq_lengths) * 0.6:
        status = "SUPPORTED"
        note = "SSM+HierGoals dominates on 50-100 step sequences"
    elif avg_ssm_delta > 0 and ssm_wins >= len(seq_lengths) * 0.6:
        status = "PARTIAL"
        note = "SSM wins, hierarchical adds marginal benefit"
    else:
        status = "REFUTED"
        note = "Concatenation competitive on 50-100 step sequences"
    ...
    with open('/tmp/H1.220_results.json', 'w') as f:
