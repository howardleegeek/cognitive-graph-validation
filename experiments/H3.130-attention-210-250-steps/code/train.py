...
results = []
for seq_len in [210, 220, 230, 240, 250]:
    attn_l, concat_l = run_exp(seq_len, 0.95)
    delta = (concat_l - attn_l) / concat_l * 100
    results.append({"seq_len": seq_len, "attn": attn_l, "concat": concat_l, "delta": delta})
    print(f"Seq={seq_len}: Attn={attn_l:.4f}, Concat={concat_l:.4f}, Δ={delta:+.1f}%")

wins = sum(1 for r in results if r["delta"] > 0)
avg = np.mean([r["delta"] for r in results])
print(f"\n{'SUPPORTED' if wins >= 3 else 'REFUTED'}: {wins}/5 wins, avg={avg:+.1f}%")
